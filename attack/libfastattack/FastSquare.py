...
class FastSquare(Attack):
    def __init__(self, model, norm='Linf', eps=None, n_queries=10, n_restarts=1,
                 p_init=.8, loss='margin', resc_schedule=True,
                 seed=0):
        super().__init__("FastSquare", model)
        self.norm = norm
        ...
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                ...
                for i_iter in range(self.n_queries):
                    ...
                    loss_min[idx_to_fool] = idx_improved.half() * loss + (
                            1. - idx_improved.half()) * loss_min_curr
                    ...
                    margin_min[idx_to_fool] = idx_improved.half() * margin + (
                            1. - idx_improved.half()) * margin_min_curr
