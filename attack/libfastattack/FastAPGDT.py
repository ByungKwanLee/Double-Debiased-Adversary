...
class FastAPGDT(Attack):

    def __init__(self, model, norm='Linf', eps=8/255, steps=10, n_restarts=1, seed=0, eot_iter=1, rho=.75, n_classes=10):
        super().__init__("FastAPGDT", model)
        self.eps = eps
        self.steps = steps
        self.norm = norm
    ...
    def dlr_loss_targeted(self, x, y, y_target):
        ...
        return -1e-3 * (x[np.arange(x.shape[0]), y] - x[np.arange(x.shape[0]), y_target])\
        / (x_sorted[:, -1] - .5 * x_sorted[:, -3] - .5 * x_sorted[:, -4] + 1e-12) + nn.CrossEntropyLoss(reduction='none')(x,y)

    def attack_single_run(self, x_in, y_in):
        x = x_in.clone() if len(x_in.shape) == 4 else x_in.clone().unsqueeze(0)
        y = y_in.clone() if len(y_in.shape) == 1 else y_in.clone().unsqueeze(0)

        self.steps_2, self.steps_min, self.size_decr = max(int(0.22 * self.steps), 1), max(int(0.06 * self.steps), 1), max(int(0.03 * self.steps), 1)  # nopep8
        ...
        for i in range(self.steps):
            ...
            with torch.no_grad():
                ...
                x_adv_1 = torch.clamp(torch.min(torch.max(x_adv_1, x - self.eps), x + self.eps), 0.0, 1.0)  # nopep8
                x_adv_1 = torch.clamp(torch.min(torch.max(x_adv + (x_adv_1 - x_adv)*a + grad2*(1 - a), x - self.eps), x + self.eps), 0.0, 1.0)  # nopep8
            ...
            with torch.no_grad():
                ...
                if counter3 == k:
                    fl_oscillation = self.check_oscillation(loss_steps.detach().cpu().numpy(), i, k, loss_best.detach().cpu().numpy(), k3=self.thr_decr)  # nopep8
            ...
            for target_class in range(2, self.n_target_classes + 2):
                ...
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        adv[ind_to_fool[ind_curr]] = adv_curr[ind_curr].half().clone()
        return acc, adv