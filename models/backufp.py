...
from utils.vit_parser import get_args
...
from timm.models.vision_transformer import resize_pos_embed, _cfg, default_cfgs, DistilledVisionTransformer
...
def checkpoint_filter_fn(state_dict, model, patch, pretrain_pos_only, patch_embed_scratch, downsample_factor):
    ...
    for k, v in state_dict.items():
        ...
        if k == 'pos_embed' and v.shape != model.pos_embed.shape:
            ...
            if True:
                ...
                v = v.reshape(*v.shape[:2], patch, v.shape[2]//patch, patch, v.shape[3]//patch).sum(dim=[3,5])

            else:
                if patch_embed_scratch:
...
def _create_vision_transformer(variant, pretrained=False, distilled=False, **kwargs):
    ...
    if pretrained:
        load_pretrained(
            model, num_classes=num_classes, in_chans=kwargs.get('in_chans', 3), filter_fn=None)
        # partial(checkpoint_filter_fn, args=kwargs.pop('args'), model=model)
    else:
...
def vit(depth, dataset, vit_type, img_size, patch_size, pretrained=False):
    ...
    model_kwargs = dict(img_size=img_size, num_classes=num_classes, patch_size=patch_size, embed_dim=hidden,
                        depth=depth, num_heads=head)

    cfg_list = ['vit_small_patch16_384', 'vit_small_patch16_224', 'vit_base_patch16_224', 'vit_base_patch32_224', 'vit_base_patch16_384', 'vit_base_patch32_384',
                'vit_base_patch16_224_in21k', 'vit_base_patch32_224_in21k', 'vit_large_patch16_224_in21k']

    model = timm.create_model('vit_%s_patch%d_%d'%(vit_type, patch_size, img_size), pretrained=pretrained)
    model.head = nn.Linear(model.head.in_features, num_classes)

    #model = _create_vision_transformer(cfg_list[0], pretrained=pretrained, **model_kwargs)
