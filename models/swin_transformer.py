...
def swin(depth, dataset, swin_type, img_size, patch_size, pretrained=False, mean=None, std=None):
    ...
    model_kwargs = dict(patch_size=patch_size, window_size=7, embed_dim=embed_dim, depth=depths, num_heads=num_heads,
                        num_classes=num_classes, mean=mean, std=std)

    model = _create_swin_transformer('swin_%s_patch%d_window7_%d'%(swin_type, patch_size, img_size), pretrained=pretrained, **model_kwargs)
