...
class TNT(nn.Module):
    ...
    def __init__(
            ...
            drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, first_stride=4, mean=None, std=None):
        super().__init__()
...
def checkpoint_filter_fn(state_dict, model):
    ...
    if state_dict['patch_pos'].shape != model.patch_pos.shape:
        state_dict['patch_pos'] = resize_pos_embed(state_dict['patch_pos'],
                                                   model.patch_pos, getattr(model, 'num_tokens', 1),
...
def tnt(depth, dataset, tnt_type, img_size, patch_size, pretrained=False, mean=None, std=None):
    ...
    model_kwargs = dict(patch_size=patch_size, embed_dim=embed_dim, depth=depth, num_heads=num_heads, in_dim=in_dim, num_classes=num_classes, mean=mean, std=std)

    model = _create_tnt('tnt_%s_patch%d_%d'%(size_type, patch_size, img_size), pretrained=pretrained, **model_kwargs)
