...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            weight_init='', embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block, mean=None, std=None):
        """
        Args:
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(VisionTransformer, variant, pretrained, pretrained_cfg=pretrained_cfg,
        pretrained_filter_fn=checkpoint_filter_fn, pretrained_custom_load='npz' in pretrained_cfg['url'], **kwargs)
    return model
...
def vit(depth, dataset, vit_type, img_size, patch_size, pretrained=False, mean=None, std=None):
    ...
    model_kwargs = dict(patch_size=patch_size, embed_dim=embed_dim, depth=depth, num_heads=num_heads, num_classes=num_classes, mean=mean, std=std)

    # model = timm.create_model('vit_%s_patch%d_%d'%(vit_type, patch_size, img_size), pretrained=pretrained)
    # model.head = nn.Linear(model.head.in_features, num_classes)

    model = _create_vision_transformer('vit_%s_patch%d_%d'%(vit_type, patch_size, img_size), pretrained=pretrained, **model_kwargs)
