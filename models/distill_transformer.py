...
def deit(depth, dataset, deit_type, img_size, patch_size, pretrained=False, mean=None, std=None):
    ...
    if deit_type == 'tiny':
        embed_dim = 192
        num_heads = 3
    elif deit_type == 'small':
        embed_dim = 384
        num_heads = 6
    elif deit_type == 'base':
        embed_dim = 768
        num_heads = 12
    elif deit_type == 'large':
        embed_dim = 1024
        num_heads = 16
    elif deit_type == 'huge':
        embed_dim = 1280
    ...
    # model = timm.create_model('deitit_%s_patch%d_%d'%(deit_type, patch_size, img_size), pretrained=pretrained)
    # model.head = nn.Linear(model.head.in_features, num_classes)

    model = _create_deit('deit_%s_distilled_patch%d_%d'%(deit_type, patch_size, img_size), pretrained=pretrained,
                         distilled=True, **model_kwargs)
