...
parser.add_argument('--learning_rate', default=1e-3, type=float) #3e-2 for ViT
...
def main_worker(rank, ngpus_per_node=ngpus_per_node):
    ...
    optimizer = optim.Adam(net.parameters(), lr=args.learning_rate, betas=(0.9, 0.999), weight_decay=5e-5)
    base_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=200, eta_min=1e-5)
    lr_scheduler = warmup_scheduler.GradualWarmupScheduler(optimizer, multiplier=1., total_epoch=5, after_scheduler=base_scheduler)
